...
import com.charlztech.tv.data.cache.ResponseCache
import com.charlztech.tv.data.model.ChannelStreamResponse
...
import com.charlztech.tv.util.CryptoUtils
import com.charlztech.tv.util.M3uParser
...
class StreamApiService {
    ...
    suspend fun resolveBaseUrl(): String = withContext(Dispatchers.IO) {
        ...
        val candidates = listOfNotNull(
            urls?.second,
            urls?.first,
            AppConfig.DEFAULT_API_BASE,
            AppConfig.FALLBACK_API_BASE
        ...
        val fallback = AppConfig.DEFAULT_API_BASE.trimEnd('/')
    ...
    private fun loadAssetPlaylist(assetPath: String): String? {
        return try {
            val stream = javaClass.classLoader.getResourceAsStream(assetPath) ?: return null
