...
object CryptoUtils {
    ...
    private val keys: List<KeyInfo> by lazy {
        listOfNotNull(
            AppConfig.CRYPTO_SECRET1.takeIf { it.isNotBlank() }?.let { parseKeyInfo(it) },
            AppConfig.CRYPTO_SECRET2.takeIf { it.isNotBlank() }?.let { parseKeyInfo(it) }
        )
    }
    ...
    fun decryptData(encryptedBase64: String): String? {
        ...
        return try {
            val clean = encryptedBase64.trim()
                ...
                .replace(" ", "")
                .replace("\t", "")
            val ciphertext = Base64.getDecoder().decode(clean)
    ...
    fun decryptContent(content: String): String {
        ...
        return try {
            ...
            val iv = Base64.getDecoder().decode(trimmed.substring(10, 34))
            val key = Base64.getDecoder().decode(trimmed.substring(trimmed.length - 54, trimmed.length - 10))
            val encryptedBytes = Base64.getDecoder().decode(encryptedDataStr)
